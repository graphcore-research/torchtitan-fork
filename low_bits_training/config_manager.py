...
class JobConfig(TTJobConfig):
    def __init__(self):
        ...
        self.parser.add_argument(
            "--profiling.consecutive_active_steps",
            type=int,
            default=1,
            help="How many steps should be collected in one go, in iterations. Increase if you need to profile how steps overlap with each other.",
        )
        self.parser.add_argument(
            "--profiling.memory_timeline",
            type=str,
            default=None,
            choices=[None, "json", "json.gz", "raw.json.gz"],
            help="Whether to profile with the memory timeline - can cause very big profiles and bugs. Choices are defined by the file formats supported by `profile.export_memory_timeline` To see how to read those files open Pytorch's _memory_profiler.py, Good luck!",
        )
        self.parser.add_argument(
            "--profiling.with_flops_table",
            action="store_true",
            help="Export flops estimate calculated by the profiler. This will slow down the profile output.",
        )
        self.parser.add_argument(
            "--profiling.experimental_cupti_stats",
            action="store_true",
            help="Turn on CUPTI hardware counters, currently broken - waiting for a fix: to https://github.com/pytorch/pytorch/issues/125272",
        )
        self.parser.add_argument(
            "--profiling.capture_compiled_kernels",
            action="store_true",
            help="Capture the triton and inductor caches generated by this model. These will be in the dump folder.",
        )
    ...
    def parse_args(self, args_list: list = sys.argv[1:]):
        ...
        # TODO: this might not be the best place to set the compilation cache
        if self.profiling.capture_compiled_kernels:
            from . import utils  # needed for circular import

            utils.set_torch_compile_env_vars(self.job.dump_folder)
    ...
    def dump(self):
        ...
        config_dict = utils.job_config_to_config_dict(self)
